...
def calc_eer():
    ...
    # キャッシュファイルの定義
    embeddings_cache_file = os.path.join("eval_dataset_results", "embeddings_cache_replay.pkl")
    ...
    files = {"ayuto": [], "hiroki": [], "olim": [], "hinako": [], "mako": []}
    for target in files:
        audios_dir = os.path.join("dataset_raw", "replay")
        ls = os.listdir(audios_dir)
        files[target] = [os.path.join(audios_dir, x) for x in ls if x.startswith(target)]

    generated_audio_files = {"ayuto": [], "hiroki": [], "olim": [], "hinako": [], "mako": []}
    for target in files:
        audios_dir = os.path.join("dataset_raw", "replay")
        ls = os.listdir(audios_dir)
        generated_audio_files[target] = [os.path.join(audios_dir, x) for x in ls if x.startswith("re_" + target)]
    ...
    # ペア数を出力
    print(f"Used {len(positive_pairs)} positive pairs.")
    print(f"Used {len(negative_pairs)} negative pairs.")
    print(f"Used {len(attack_pairs)} attacking pairs.")

    # EER計算
    eer_index_no_attack = np.nanargmin(np.abs(fpr_no_attack - fnr))
    eer_no_attack = fpr_no_attack[eer_index_no_attack]
    eer_threshold_no_attack = thresholds[eer_index_no_attack]

    eer_index = np.nanargmin(np.abs(fpr - fnr))
    eer = fpr[eer_index]
    eer_threshold = thresholds[eer_index]

    print(f"Equal Error Rate (without Attacking): {eer_no_attack * 100:.2f}%")
    print(f"Threshold: {eer_threshold_no_attack}")
    print(f"Equal Error Rate (with Attacking): {eer * 100:.2f}%")
    ...
    # plt.legend()
    plt.grid()
    plt.savefig(os.path.join("eval_dataset_results", "eer_replay.png"))

    plt.figure()
    plt.plot(thresholds, fpr, label="FAR (with attacking)", color="#F24343")
    plt.plot(thresholds, fpr_no_attack, label="FAR (no attacking)", color="#4285F4")
    plt.plot(thresholds, fnr, label="FRR", color="#F2AC43")
    # plt.axvline(eer_threshold, color="red", linestyle="--", label=f"EER = {eer * 100:.2f}% at Threshold = {eer_threshold:.2f}")
    plt.xlabel("Threshold")
    plt.ylabel("Error Rate")
    # plt.title("FAR / FRR")
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join("eval_dataset_results", "eer_replay_with_legend.png"))
